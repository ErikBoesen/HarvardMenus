...
struct LocationView: View {
    ...
    var body: some View {
        VStack {
            Header(text: self.model.location.name, location: self.model.location)
            if self.model.meals[self.model.date] != nil {
                if self.model.meals[self.model.date]!.isEmpty {
                    // TODO: reenable
//                    SplashView(image: self.model.location.id, subtitle: "No menu posted.")
                    SplashView(image: "BK", subtitle: "No menu posted.")
                } else {
                    // TODO: reenable
//                    SegmentedPicker(items: self.model.meals[self.model.date]!.map { $0.name }, selection: $model.mealIndex.onChange(onChange))
//                        .padding(.bottom, 10)
                    // TODO: reenable if we can get meal times reliably
...
//                        .foregroundColor(.main)
//                        .padding(.bottom, 4)
                    if self.model.recipes[self.model.date] != nil &&
                       self.model.recipes[self.model.date]![self.model.mealIndex] != nil &&
                       self.model.allowed[self.model.date]![self.model.mealIndex] != nil {
                        ScrollView {
                            VStack {
                                ForEach(
                                    Array(zip(
                                        self.model.recipes[self.model.date]![self.model.mealIndex]!,
                                        self.model.allowed[self.model.date]![self.model.mealIndex]!
                                    )),
                                    id: \.0.id
                                ) { recipe, allowed in
                                    RecipePreviewView(recipe: recipe, allowed: allowed)
                                }
                            }
                        }
                    } else {
    ...
    func onChange(mealIndex: Int) {
        ...
        self.model.getRecipes(date: self.model.date, mealIndex: mealIndex)
    }
}
